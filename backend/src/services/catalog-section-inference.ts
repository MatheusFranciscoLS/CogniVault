...
const RULES: Rule[] = [
  { section: 'CRANKCASE & CLUTCHDRUM', patterns: [
  ...
  { section: 'CYLINDER PISTON', patterns: [
...
export function inferCatalogSection(rows: CatalogSectionEvidence[]): string {
  ...
  for (const rule of RULES) {
    let score = 0;
    for (const item of rule.patterns) if (item.pattern.test(evidence)) score += item.weight;
    if (score > bestScore) {
      bestScore = score;
