...
function explicitTechnicalContradictions(question: string, candidate: PartCandidate): string[] {
  ...
  const candidateText = [candidate.name, candidate.section, ...candidate.alternativeNames, candidate.notes].filter(Boolean).join(' ');
  const present = extractTechnicalQualifiers(candidateText);
  const labels: Array<[keyof typeof requested, string]> = [
    ['direction', 'lado LH/RH'], ['axlePosition', 'posição dianteira/traseira'], ['sprocketType', 'tipo Rim/Spur'],
    ['teeth', 'quantidade de dentes'], ['chainPitch', 'passo da corrente'], ['metricThread', 'rosca métrica'],
    ['metricSizeMm', 'medida em milímetros'], ['voltageV', 'tensão elétrica'], ['inchSize', 'medida em polegadas'],
  ];
  const conflicts: string[] = [];
  for (const [key, label] of labels) {
    if (requested[key] !== null && present[key] !== null && requested[key] !== present[key]) conflicts.push(label);
  }
  const requestedPosition = explicitPosition(question);
  const candidatePosition = candidate.position?.trim() || null;
  if (requestedPosition && candidatePosition && requestedPosition !== candidatePosition) conflicts.push('posição da vista explodida');
  if (serialApplicability(question, {
    name: candidate.name, section: candidate.section, aliases: candidate.alternativeNames, notes: candidate.notes, pnc: candidate.pnc,
  }) === 'CONFLICT') conflicts.push('número de série');
  return conflicts;
}

function catalogEvidence(context: CatalogConfidenceContext | undefined): { adjustment:number; blocksInference:boolean; messages:string[] } {
  ...
  const health = typeof context.healthScore === 'number' && Number.isFinite(context.healthScore)
    ? Math.max(0, Math.min(100, context.healthScore)) : null;
  const status = context.reviewStatus || null;
  const messages:string[]=[];
  let adjustment=0;
  let blocksInference=false;
  if (status === 'NEEDS_REVIEW') {
    blocksInference=true; adjustment-=0.16;
    ...
    adjustment+=0.03; messages.push('O catálogo de origem já passou por revisão administrativa de qualidade.');
  } else if (status === 'READY') {
    adjustment+=0.01; messages.push('O catálogo de origem passou pelas verificações estruturais automáticas.');
  } else if (status === 'PENDING') {
    adjustment-=0.02; messages.push('A revisão estrutural do catálogo de origem ainda está pendente.');
  }
  if (health !== null && health > 0) {
    if (health < 60) { blocksInference=true; adjustment-=0.12; messages.push(`Saúde estrutural do catálogo: ${Math.round(health)}/100, abaixo do mínimo para liberar uma inferência.`); }
    else if (health < 75) { adjustment-=0.04; messages.push(`Saúde estrutural do catálogo: ${Math.round(health)}/100; confiança reduzida.`); }
    else if (health >= 90) { adjustment+=0.02; messages.push(`Saúde estrutural do catálogo: ${Math.round(health)}/100.`); }
  }
  if (blocksInference && context.reviewReasons?.length) messages.push(`Motivo de revisão: ${context.reviewReasons[0]}.`);
  return { adjustment, blocksInference, messages };
}

/** Ranking e autorização são separados: primeiro lugar não significa permissão para liberar código. */
export function evaluateAnswerConfidence(params: {
  ...
  if (exactCode || chosen.searchMethod === 'DIRECT_CODE') {
    return {
      safe:true, confidence:1, level:'EXACT',
  ...
  const specificity=relationSpecificityBonus(question,{name:chosen.name,section:chosen.section,aliases:chosen.alternativeNames,notes:chosen.notes,pnc:chosen.pnc});
  const qualifierCount=explicitQualifierCount(question);
  const serial=serialApplicability(question,{name:chosen.name,section:chosen.section,aliases:chosen.alternativeNames,notes:chosen.notes,pnc:chosen.pnc});
  ...
  const rawRetrievalMargin=runnerUp&&!sameCodeRunner?retrieval-retrievalEvidence(runnerUp):1;
  const serialNeedsConfirmation=Boolean(runnerUp && requiresSerialConfirmation(question,[chosen,runnerUp]));
  const differentCodeNearTie=Boolean(runnerUp&&!sameCodeRunner&&rawRetrievalMargin<0.03&&margin<0.16&&serial!=='MATCH');

  let confidence=Math.min(clamp(params.selectionConfidence),clamp(Math.max(semanticConfidence,retrieval)));
  if(agreement>=3)confidence+=0.10; else if(agreement===2)confidence+=0.06;
  ...
  if(serialNeedsConfirmation)confidence=Math.min(confidence,0.69);
  ...
  const safe=confidence>=0.72&&strongLead&&independentEvidence&&!differentCodeNearTie&&!serialNeedsConfirmation&&contradictions.length===0&&!fuzzyOnly&&!catalog.blocksInference;
  const evidence:string[]=[];
  if(agreement>=2)evidence.push(`${agreement} métodos independentes concordaram com esta peça.`); else evidence.push(`Recuperação principal: ${retrievalSources[0]||chosen.searchMethod}.`);
  ...
  if(serialNeedsConfirmation)evidence.push('A mesma posição possui códigos diferentes separados por faixas explícitas de número de série.');
  if(runnerUp&&!sameCodeRunner){evidence.push(`Margem técnica sobre a segunda opção: ${Math.max(0,margin).toFixed(3)}.`);evidence.push(`Margem entre recuperadores para códigos diferentes: ${Math.max(0,rawRetrievalMargin).toFixed(3)}.`);}
  ...
  let reason='A evidência ainda não é suficiente para liberar automaticamente um código.';
  if(safe)reason='Há evidência independente, separação suficiente das alternativas e o catálogo não possui bloqueio estrutural conhecido.';
  else if(contradictions.length)reason='O candidato contradiz um detalhe técnico informado explicitamente na consulta; não vou liberar o código.';
  else if(serialNeedsConfirmation)reason='Esta peça possui variantes da mesma posição separadas por número de série. Informe o S/N da máquina para eu liberar o código correto.';
  else if(catalog.blocksInference)reason='O catálogo de origem precisa de revisão antes de uma resposta inferida ser liberada automaticamente.';
  else if(differentCodeNearTie)reason='Dois códigos diferentes ficaram próximos demais para eu liberar um deles automaticamente.';

  return {safe,confidence,level:safe&&confidence>=0.86?'HIGH':'REVIEW',evidence,reason};
}
