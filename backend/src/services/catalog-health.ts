 ...
 * é tratada como revisão humana porque pode trocar aplicação de peça.
 */
...
export async function refreshCatalogHealth(documentId: string, tenantId: string): Promise<CatalogHealth | null> {
  ...
  const [partCount, partsWithPage, partsWithSection, embeddedPartCount] = await Promise.all([
    prisma.part.count({ where: { documentId, active: true } }),
