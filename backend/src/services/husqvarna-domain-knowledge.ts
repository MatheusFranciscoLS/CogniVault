...
const MODEL_FAMILIES: ModelFamilyEntry[] = [
  ...
  // Sopradores analisados. O PDF recebido como “345BT” identifica-se como 340BT.
...
const DOMAIN_RULES: DomainRule[] = [
  ...
  { key: 'rocker-arm', families: ['ENGINE'], triggers: ['balancim', 'balancins', 'rocker arm'], technicalTerms: ['rocker arm', 'rocker arm kit', 'kit da valvula rocker arm'] },
  { key: 'push-rod', families: ['ENGINE'], triggers: ['vareta de valvula', 'vareta da valvula', 'haste de valvula', 'push rod'], technicalTerms: ['push rod', 'push rod set', 'biela push rod'] },
  { key: 'camshaft', families: ['ENGINE'], triggers: ['arvore de cames', 'arvore do comando', 'eixo comando', 'comando de valvulas', 'camshaft'], technicalTerms: ['camshaft', 'camshaft kit', 'arvore de cames'] },
  { key: 'governor', families: ['ENGINE'], triggers: ['governador', 'regulador de giro', 'regulador de rotacao', 'governor'], technicalTerms: ['governor', 'governor gear', 'speed regulating arm'] },
  { key: 'oil-strainer', families: ['ENGINE'], triggers: ['peneira de oleo', 'pescador de oleo', 'tela de oleo', 'oil strainer'], technicalTerms: ['oil strainer', 'peneira de oleo', 'strainer'] },
  { key: 'voltage-rectifier', families: ['ENGINE'], triggers: ['retificador', 'regulador de tensao', 'regulador de voltagem', 'voltage rectifier'], technicalTerms: ['voltage rectifier', 'rectifier kit', 'rectifier support'] },
  ...
  { key: 'bar-adjuster', families: ['CHAINSAW', 'POLE_PRUNER'], triggers: ['ajustador do sabre', 'regulador do sabre', 'bar adjuster'], technicalTerms: ['bar adjuster', 'chain adjuster', 'chain tensioner', 'esticador de corrente'] },
  { key: 'rim-sprocket', families: ['CHAINSAW'], triggers: ['rim'], technicalTerms: ['rim', 'rim sprocket'] },
  { key: 'spur-sprocket', families: ['CHAINSAW'], triggers: ['spur'], technicalTerms: ['spur', 'spur sprocket'] },
  ...
  { key: 'belt-faixa', families: ['GARDEN_TRACTOR', 'ZERO_TURN', 'RIDER'], triggers: ['faixa', 'faixa do deck', 'faixa da tracao'], technicalTerms: ['belt', 'drive belt', 'deck drive', 'ground drive'] },
  ...
  { key: 'zero-turn-hydro', families: ['ZERO_TURN'], triggers: ['transmissao', 'cambio', 'hidrostatica', 'hidrostatico', 'hydro'], technicalTerms: ['transmission', 'transmission complete', 'hydrostatic', 'hte', 'ezt'] },
...
function knownModelFromText(value: string): ModelFamilyEntry | null {
  ...
  // Modelos só numéricos exigem limite de palavra para não casar um Part Number.
  for (const entry of MODEL_FAMILIES.filter(item => /^\d+$/.test(item.model))) {
    if (new RegExp(`(^|\\s)${entry.model}(?=\\s|$)`, 'i').test(text)) return entry;
  }

  const candidates = MODEL_FAMILIES
    .filter(entry => !/^\d+$/.test(entry.model))
    .sort((a, b) => normalizeIdentifier(b.model).length - normalizeIdentifier(a.model).length);
  return candidates.find(entry => identifier.includes(normalizeIdentifier(entry.model))) || null;
}
