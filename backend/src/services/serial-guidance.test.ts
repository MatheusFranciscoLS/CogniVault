...
test('pede serial quando o mesmo item troca de código por fronteira explícita de S/N', () => {
  assert.equal(
    requiresSerialConfirmation('adaptador da lâmina LB256SP PNC 970488501', [oldAdapter, newAdapter]),
    true,
  );
});
...
test('não pede serial novamente quando o usuário já informou S/N', () => {
  assert.equal(
    requiresSerialConfirmation('adaptador da lâmina LB256SP PNC 970488501 S/N 20240200001', [oldAdapter, newAdapter]),
    false,
  );
});
...
test('não mistura regras de posições diferentes como se fossem variantes do mesmo item', () => {
  assert.equal(
    requiresSerialConfirmation('peça do LB256SP', [oldAdapter, { ...newAdapter, position: '7' }]),
    false,
  );
});
...
test('não mistura PNCs diferentes para fabricar uma necessidade de serial', () => {
  assert.equal(
    requiresSerialConfirmation('adaptador LB256SP', [oldAdapter, { ...newAdapter, pnc: '970488502' }]),
    false,
  );
});
