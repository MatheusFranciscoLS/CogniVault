...
function isPncListMisreadAsPart(row:ParsedRow,knownPncs:string[]){const code=normalizeHusqvarnaPnc(row.partNumber);if(!code||!knownPncs.includes(code))return false;const withoutPncs=comparable([row.name,row.comments].filter(Boolean).join(' ')).replace(/\b\d{9,11}\b/g,'').replace(/[^a-z]+/g,'');return withoutPncs.length<3;}
