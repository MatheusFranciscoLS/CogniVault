...
const HUSQVARNA_ROW=new RegExp(`^(\\d{1,3}(?:[a-zA-Z]|-\\d{1,2})?)\\s+(${SPACED_PART_NUMBER_PATTERN})\\s+(.+?)\\s+([A-Z])\\s+(\\d+)(?:\\s+(.+))?$`,'i');
const GENERIC_PART_ROW=new RegExp(`^(\\d{1,3}(?:[a-zA-Z]|-\\d{1,2})?)\\s+(${SPACED_PART_NUMBER_PATTERN})\\s+(.+?)\\s+(\\d+)(?:\\s+(.+))?$`,'i');
const FLEXIBLE_ROW_START=new RegExp(`^(\\d{1,3}(?:[a-zA-Z]|-\\d{1,2})?)\\s+(${PART_NUMBER_PATTERN})\\s*(.*)$`,'i');
