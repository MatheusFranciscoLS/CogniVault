...
export default function AdminFeedbackPanel(){
  ...
  return <section>
    <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="cv-kicker">Qualidade da busca</p><h1 className="cv-page-title">Feedback da IA</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Acompanhe confirmações e erros reportados pelo balcão para identificar onde o ranking precisa melhorar.</p></div><button type="button" disabled={loading} onClick={()=>void load()} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-600 disabled:opacity-50">{loading?'Atualizando…':'Atualizar dados'}</button></div>
