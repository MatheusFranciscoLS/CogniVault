    ...
    <section className="mx-auto max-w-[1400px] space-y-4">
      ...
      <div className="overflow-hidden rounded-xl border border-ink-200 bg-white dark:border-ink-800 dark:bg-ink-900">
        <div className="grid divide-y divide-ink-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0 xl:grid-cols-4 dark:divide-ink-800">
          <div className="px-4 py-4"><div className="text-[10px] font-black uppercase tracking-[.1em] text-ink-400">Sinais independentes</div><div className="mt-2 text-2xl font-black">{loading ? '—' : summary.uniqueSignals}</div><div className="mt-1 text-[11px] text-ink-400">{summary.total} avaliações registradas</div></div>
          <div className="px-4 py-4"><div className="text-[10px] font-black uppercase tracking-[.1em] text-ink-400">Confirmados</div><div className="mt-2 text-2xl font-black text-emerald-700 dark:text-emerald-300">{loading ? '—' : summary.positive}</div><div className="mt-1 text-[11px] text-ink-400">Resultados aprovados</div></div>
          <div className="px-4 py-4"><div className="text-[10px] font-black uppercase tracking-[.1em] text-ink-400">Corrigidos</div><div className="mt-2 text-2xl font-black text-ink-900 dark:text-brand-300">{loading ? '—' : summary.corrected}</div><div className="mt-1 text-[11px] text-ink-400">Peça correta informada</div></div>
          <div className="px-4 py-4"><div className="text-[10px] font-black uppercase tracking-[.1em] text-ink-400">Base</div><div className="mt-2 text-lg font-black text-ink-900 dark:text-white">{loading ? '—' : learningLabel}</div><div className="mt-1 text-[11px] text-ink-400">{summary.nextMilestone ? `Próximo estágio: ${summary.nextMilestone} sinais` : 'Consenso estabelecido'}</div></div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-ink-200 bg-white p-3 dark:border-ink-800 dark:bg-ink-900">
        ...
        <span className="text-xs font-semibold text-ink-400">{visible.length} registros</span>
      </div>

      <div className="overflow-x-auto rounded-xl border border-ink-200 bg-white dark:border-ink-800 dark:bg-ink-900">
        <table className="w-full min-w-[980px] text-sm">
          <thead className="border-b border-ink-100 bg-ink-50/70 text-left text-[10px] font-black uppercase tracking-[.1em] text-ink-400 dark:border-ink-800 dark:bg-ink-800/50"><tr><th className="px-4 py-3">Consulta</th><th>Resultado</th><th>Avaliação</th><th>Usuário</th><th>Data</th><th className="px-4 text-right">Ação</th></tr></thead>
          <tbody>
            {visible.map(item => (
              <tr key={item.id} className="border-b border-ink-100 align-top last:border-0 hover:bg-ink-50/70 dark:border-ink-800 dark:hover:bg-ink-800/40">
                <td className="px-4 py-3"><div className="max-w-[320px] font-semibold text-ink-800 dark:text-ink-100">{item.query}</div><div className="mt-1 text-[11px] text-ink-400">PNC {item.pnc || 'não informado'}</div></td>
                <td className="py-3"><div className="font-mono text-xs font-black text-ink-900 dark:text-brand-300">{item.resultPart?.partNumber || '—'}</div><div className="mt-1 max-w-[250px] text-[11px] text-ink-400">{item.resultPart?.name || 'Peça indisponível'}</div>{item.correctedPart && <div className="mt-1 text-[11px] font-semibold text-brand-700 dark:text-brand-300">Correta: {item.correctedPart.partNumber} · {item.correctedPart.name}</div>}</td>
                <td className="py-3"><span className={`text-xs font-bold ${item.correct ? 'text-emerald-700 dark:text-emerald-300' : 'text-rose-700 dark:text-rose-300'}`}>{item.correct ? 'Correto' : 'Incorreto'}</span>{!item.correct && <div className="mt-1 text-[11px] text-ink-400">{item.reason ? reasonLabel[item.reason] || item.reason : 'Sem motivo'}</div>}</td>
                <td className="py-3 text-xs text-ink-500 dark:text-ink-400">{item.user?.email || 'Sistema'}</td>
                <td className="py-3 text-xs text-ink-400">{fmtDate(item.createdAt)}</td>
                <td className="px-4 py-3 text-right"><button type="button" disabled={deletingId === item.id} onClick={() => void deleteFeedback(item.id)} className="rounded-lg px-2.5 py-2 text-xs font-bold text-ink-400 transition hover:bg-rose-50 hover:text-rose-600 disabled:opacity-50 dark:hover:bg-rose-950/30">{deletingId === item.id ? 'Excluindo…' : 'Excluir'}</button></td>
              </tr>
            ))}
          </tbody>
        </table>
        {loading ? <div className="p-10 text-center text-sm text-ink-400">Carregando feedbacks…</div> : !visible.length ? <div className="p-10 text-center text-sm text-ink-400">Nenhum feedback neste filtro.</div> : null}
      </div>
    </section>
  );
}
