  ...
  return (
    <section className="mx-auto max-w-[1400px] space-y-4">
      <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="text-xs font-semibold text-ink-400">{history.length} {history.length === 1 ? 'consulta' : 'consultas'}</div>
      </div>
      ...
      <div className="flex flex-col gap-2 rounded-xl border border-ink-200 bg-white p-3 shadow-sm dark:border-ink-800 dark:bg-ink-900 sm:flex-row sm:items-center">
        ...
        <div className="px-1 text-xs font-semibold text-ink-400">{filtered.length} registros</div>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-ink-200 bg-white dark:border-ink-800 dark:bg-ink-900">
        <div className="hidden grid-cols-[minmax(250px,1.35fr)_minmax(230px,1fr)_120px_130px] gap-4 border-b border-ink-100 px-4 py-2.5 text-[10px] font-black uppercase tracking-[.1em] text-ink-400 lg:grid dark:border-ink-800">
                ...
                return (
                  <article key={virtualItem.key} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: `${virtualItem.size}px`, transform: `translateY(${virtualItem.start}px)` }} className="grid gap-3 border-b border-ink-100 px-4 py-3 transition hover:bg-ink-50/80 lg:grid-cols-[minmax(250px,1.35fr)_minmax(230px,1fr)_120px_130px] lg:items-center dark:border-ink-800 dark:hover:bg-ink-800/45">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 truncate text-[11px] text-ink-400">{fmtDate(item.createdAt)}{item.pnc ? ` · PNC ${item.pnc}` : ''}</div>
                    </div>
                    <div className="min-w-0">
                      {item.resultCode ? <><div className="truncate font-mono text-sm font-black text-ink-900 dark:text-brand-300">{code}</div><div className="mt-1 truncate text-[11px] text-ink-400">{item.resultLabel || 'Peça'}{item.resultModel ? ` · ${item.resultModel}` : ''}</div></> : <span className="text-xs text-ink-400">Sem código confirmado</span>}
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 lg:justify-end">
                      ...
                      {item.resultCode && <button type="button" onClick={() => void copy(code)} className="rounded-lg px-2.5 py-2 text-xs font-bold text-ink-400 transition hover:bg-ink-100 hover:text-brand-600 dark:hover:bg-ink-800" title="Copiar código">Copiar</button>}
        ...
        ) : <div className="px-5 py-12 text-center"><div className="text-sm font-bold text-ink-700 dark:text-ink-200">Nenhuma consulta encontrada</div><p className="mt-1 text-xs text-ink-400">Ajuste o texto ou a situação.</p></div>}
      </div>
    </section>
  );
}
