  ...
  return (
    <section className="mx-auto max-w-[1400px] space-y-4">
      ...
      <div className="flex flex-col gap-2 rounded-xl border border-ink-200 bg-white p-3 shadow-sm dark:border-ink-800 dark:bg-ink-900 sm:flex-row sm:items-center">
        ...
        <div className="px-1 text-xs font-semibold text-ink-400">{filtered.length} itens</div>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-ink-200 bg-white dark:border-ink-800 dark:bg-ink-900">
        <div className="hidden grid-cols-[90px_minmax(220px,1.25fr)_150px_minmax(170px,.8fr)_170px] gap-4 border-b border-ink-100 px-4 py-2.5 text-[10px] font-black uppercase tracking-[.1em] text-ink-400 lg:grid dark:border-ink-800">
        ...
        {loading ? <div className="space-y-1 p-3">{[0,1,2,3].map(item => <div key={item} className="h-16 animate-pulse rounded-lg bg-ink-100 dark:bg-ink-800" />)}</div> : filtered.length ? filtered.map(item => {
          ...
          return (
            <article key={item.id} className="grid gap-3 border-b border-ink-100 px-4 py-3.5 last:border-0 transition hover:bg-ink-50/80 lg:grid-cols-[90px_minmax(220px,1.25fr)_150px_minmax(170px,.8fr)_170px] lg:items-center dark:border-ink-800 dark:hover:bg-ink-800/45">
              <div className="text-[10px] font-black uppercase tracking-[.1em] text-ink-400">{item.kind === 'PART' ? 'Peça' : 'Catálogo'}</div>
              ...
              <div className="text-xs text-ink-500 dark:text-ink-400">{item.model || '—'}{item.pnc ? <><br /><span className="text-[11px] text-ink-400">PNC {item.pnc}</span></> : null}</div>
              <div className="min-w-0 truncate text-xs text-ink-400">{item.sourceFilename || item.section || '—'}</div>
              <div className="flex flex-wrap items-center gap-1.5 lg:justify-end">
                ...
                {item.reference && <button type="button" onClick={() => void copy(code)} className="rounded-lg px-2.5 py-2 text-xs font-bold text-ink-400 transition hover:bg-ink-100 hover:text-brand-600 dark:hover:bg-ink-800">Copiar</button>}
                ...
                <button type="button" onClick={() => void remove(item.id)} className="rounded-lg px-2.5 py-2 text-xs font-bold text-ink-400 transition hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-950/30">Remover</button>
              </div>
            </article>
          );
        }) : <div className="px-5 py-12 text-center"><div className="text-sm font-bold text-ink-700 dark:text-ink-200">Nenhum favorito encontrado</div><p className="mt-1 text-xs text-ink-400">Favorite uma peça ou catálogo para acessá-lo rapidamente aqui.</p></div>}
      </div>
