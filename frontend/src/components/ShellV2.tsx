...
function SidebarContent({ user, section, onSelect, onLogout, theme, onToggleTheme, soundEnabled, onToggleSound }: { user:SessionUser; section:Section; onSelect:(section:Section)=>void; onLogout:()=>void; theme:string; onToggleTheme:()=>void; soundEnabled:boolean; onToggleSound:()=>void }) {
  ...
  return <>
    ...
    <div className="flex h-[68px] items-center gap-3 border-b border-ink-200 px-4 dark:border-ink-800"><div className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-lg bg-brand-600"><img src="/favicon.png" alt="" className="h-9 w-9 object-cover"/></div><div className="min-w-0"><div className="text-sm font-black tracking-tight text-ink-950 dark:text-white">CogniVault</div><div className="mt-0.5 flex items-center gap-1.5"><span className="rounded bg-accent-50 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-[.1em] text-accent-700 dark:bg-accent-500/15 dark:text-accent-300">Balcão · Peças</span></div></div></div>
    <div className="flex-1 overflow-y-auto px-3 py-4"><div className="px-3 pb-2 text-[10px] font-black uppercase tracking-[.15em] text-ink-400">Operação</div><nav className="space-y-1">{primaryNav.map(item => <NavButton key={item.id} item={item} active={section === item.id || ((section === 'home' || section === 'assistant') && item.id === 'parts')} onSelect={onSelect}/>)}</nav><div className="my-3 h-px bg-ink-200 dark:bg-ink-800"/><CollapsibleNav label="Mais" items={secondaryNav} open={showMore} onToggle={() => setMoreOpen(value => !value)} section={section} onSelect={onSelect}/>{user.role === 'ADMIN' && <><div className="my-3 h-px bg-ink-200 dark:bg-ink-800"/><CollapsibleNav label="Administração" items={adminNav} open={showAdmin} onToggle={() => setAdminOpen(value => !value)} section={section} onSelect={onSelect}/></>}</div>
    <div className="border-t border-ink-200 p-3 dark:border-ink-800"><div className="flex items-center gap-2.5 px-1"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-ink-900 text-[10px] font-black text-white">{user.email.slice(0,2).toUpperCase()}</div><div className="min-w-0 flex-1"><div className="truncate text-[11px] font-bold text-ink-800 dark:text-ink-100">{user.email}</div><div className="mt-0.5 text-[9px] font-semibold uppercase tracking-wide text-ink-400">{user.role === 'ADMIN' ? 'Administrador' : 'Operação'}</div></div></div><div className="mt-3 grid grid-cols-3 gap-1.5"><button type="button" onClick={onToggleTheme} className="grid h-8 place-items-center rounded-lg border border-ink-200 text-ink-500 dark:border-ink-700 dark:text-ink-300" aria-label="Alternar tema"><Icon name={theme === 'dark' ? 'sun' : 'moon'} className="h-4 w-4"/></button><button type="button" onClick={onToggleSound} className="grid h-8 place-items-center rounded-lg border border-ink-200 text-ink-500 dark:border-ink-700 dark:text-ink-300" aria-label={soundEnabled ? 'Desativar sons' : 'Ativar sons'}><Icon name={soundEnabled ? 'sound' : 'mute'} className="h-4 w-4"/></button><button type="button" onClick={onLogout} className="grid h-8 place-items-center rounded-lg border border-ink-200 text-ink-500 hover:text-rose-600 dark:border-ink-700 dark:text-ink-300" aria-label="Sair"><Icon name="logout" className="h-4 w-4"/></button></div></div>
  </>;
}
...
export default function ShellV2({ user, section, onSection, onLogout, onSearch, children }: Props) {
  ...
  return <div className="min-h-screen bg-ink-100 text-ink-900 dark:bg-ink-950 dark:text-ink-100">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[210px] flex-col border-r border-ink-200 bg-white md:flex dark:border-ink-800 dark:bg-ink-900"><SidebarContent user={user} section={section} onSelect={select} onLogout={onLogout} theme={theme} onToggleTheme={toggleTheme} soundEnabled={soundEnabled} onToggleSound={toggleSoundPreference}/></aside>
    {mobileOpen && <><button type="button" aria-label="Fechar menu" onClick={() => setMobileOpen(false)} className="fixed inset-0 z-40 bg-ink-950/45 backdrop-blur-sm md:hidden"/><aside className="fixed inset-y-0 left-0 z-50 flex w-[280px] max-w-[88vw] flex-col bg-white shadow-2xl md:hidden dark:bg-ink-900"><button type="button" onClick={() => setMobileOpen(false)} className="absolute right-3 top-4 grid h-9 w-9 place-items-center rounded-lg border border-ink-200 bg-white text-ink-500 dark:border-ink-700 dark:bg-ink-800" aria-label="Fechar menu"><Icon name="close" className="h-4 w-4"/></button><SidebarContent user={user} section={section} onSelect={select} onLogout={onLogout} theme={theme} onToggleTheme={toggleTheme} soundEnabled={soundEnabled} onToggleSound={toggleSoundPreference}/></aside></>}
    <div className="min-h-screen md:pl-[210px]"><header className="sticky top-0 z-30 border-b border-ink-200/90 bg-white/95 backdrop-blur-xl dark:border-ink-800 dark:bg-ink-950/95"><div className="flex h-[68px] items-center gap-3 px-4 lg:px-5"><button type="button" onClick={() => setMobileOpen(true)} className="grid h-10 w-10 shrink-0 place-items-center rounded-lg border border-ink-200 bg-white text-ink-600 md:hidden dark:border-ink-700 dark:bg-ink-900" aria-label="Abrir menu"><Icon name="menu" className="h-[18px] w-[18px]"/></button><div className="hidden min-w-[110px] xl:block"><div className="text-[9px] font-bold uppercase tracking-[.14em] text-ink-400">Área de trabalho</div><div className="mt-0.5 text-sm font-black text-ink-900 dark:text-white">{currentTitle}</div></div>{isCounter ? <div className="flex min-w-0 flex-1 items-center gap-2 overflow-hidden"><span className={`h-2 w-2 shrink-0 rounded-full ${quoteCart.totalItems > 0 || hasContext || session.customerName ? 'bg-emerald-500' : 'bg-ink-300 dark:bg-ink-600'}`} /><span className="truncate text-xs font-semibold text-ink-500 dark:text-ink-400">{session.machineModel || session.pnc || session.customerName ? [session.customerName, session.machineModel, session.pnc ? `PNC ${session.pnc}` : ''].filter(Boolean).join(' · ') : 'Atendimento pronto para iniciar'}</span><span className="hidden text-[10px] text-ink-300 lg:inline">Ctrl K busca</span></div> : <form onSubmit={submitSearch} role="search" className="min-w-0 flex-1"><div className="relative mx-auto max-w-3xl"><Icon name="search" className="pointer-events-none absolute left-3.5 top-1/2 h-[17px] w-[17px] -translate-y-1/2 text-ink-400"/><input id="cv-workspace-search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Buscar código, descrição, modelo ou PNC" className="h-11 w-full rounded-lg border border-ink-200 bg-ink-50 pl-10 pr-20 text-sm font-medium outline-none transition placeholder:text-ink-400 focus:border-brand-600 focus:bg-white focus:ring-4 focus:ring-brand-500/10 dark:border-ink-700 dark:bg-ink-900 dark:focus:bg-ink-900"/><span className="pointer-events-none absolute right-3 top-1/2 hidden -translate-y-1/2 rounded border border-ink-200 bg-white px-2 py-1 text-[9px] font-bold text-ink-400 lg:block dark:border-ink-700 dark:bg-ink-800">Ctrl K</span></div></form>}<button type="button" onClick={() => quoteCart.setIsOpen(true)} className={`flex h-10 shrink-0 items-center gap-2 rounded-lg border px-3 text-xs font-bold transition ${quoteCart.totalItems > 0 ? 'border-amber-300 bg-amber-50 text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200' : 'border-ink-200 bg-white text-ink-600 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-300'}`}><Icon name="quote" className="h-4 w-4"/><span className="hidden sm:inline">Orçamento</span>{quoteCart.totalItems > 0 && <span className="rounded-full bg-ink-900 px-1.5 py-0.5 text-[9px] text-white">{quoteCart.totalItems}</span>}</button><Notifications items={notifications} open={notificationsOpen} onToggle={() => setNotificationsOpen(value => !value)} onClose={() => setNotificationsOpen(false)}/></div></header><main className={`mx-auto w-full px-4 py-5 lg:px-5 lg:py-6 ${isCounter ? 'max-w-[1800px]' : 'max-w-[1500px]'}`}>{children}</main></div>
  </div>;
}
