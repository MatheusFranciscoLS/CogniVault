...
function SidebarContent({ user, section, onSelect, onLogout, theme, onToggleTheme, soundEnabled, onToggleSound }: { user:SessionUser; section:Section; onSelect:(section:Section)=>void; onLogout:()=>void; theme:string; onToggleTheme:()=>void; soundEnabled:boolean; onToggleSound:()=>void }) {
  ...
  return <>
    <div className="flex h-[68px] items-center gap-3 border-b border-ink-200 px-4 dark:border-ink-800"><div className="grid h-9 w-9 place-items-center rounded-lg bg-brand-600"><img src="/favicon.png" alt="" className="h-6 w-6 rounded object-cover"/></div><div className="min-w-0"><div className="text-sm font-black tracking-tight text-ink-950 dark:text-white">CogniVault</div><div className="mt-0.5 text-[9px] font-bold uppercase tracking-[.14em] text-brand-600 dark:text-brand-300">Vardão Máquinas</div></div></div>
