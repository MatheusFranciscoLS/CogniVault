...
type IconName = 'home'|'search'|'catalog'|'quote'|'history'|'favorite'|'dashboard'|'users'|'feedback'|'quality'|'audit'|'bell'|'sun'|'moon'|'sound'|'mute'|'logout'|'menu'|'close'|'chevron';

const primaryNav: NavItem[] = [
  { id: 'home', label: 'Início', icon: 'home' },
  { id: 'parts', label: 'Peças', icon: 'search' },
  { id: 'catalogs', label: 'Catálogos', icon: 'catalog' },
  { id: 'quotes', label: 'Orçamento', icon: 'quote' },
];
...
function SidebarContent({ user, section, onSelect, onLogout, theme, onToggleTheme, soundEnabled, onToggleSound }: { user:SessionUser; section:Section; onSelect:(section:Section)=>void; onLogout:()=>void; theme:string; onToggleTheme:()=>void; soundEnabled:boolean; onToggleSound:()=>void }) {
  ...
  useEffect(() => { if (secondaryActive) setMoreOpen(true); }, [secondaryActive]);
  ...
  return <>
    ...
    <div className="flex-1 overflow-y-auto px-3 py-4"><div className="px-3 pb-2 text-[10px] font-black uppercase tracking-[.15em] text-slate-400">Operação</div><nav className="space-y-1">{primaryNav.map(item => <NavButton key={item.id} item={item} active={section === item.id} onSelect={onSelect}/>)}</nav><div className="my-3 h-px bg-slate-200 dark:bg-slate-800"/><CollapsibleNav label="Mais" items={secondaryNav} open={moreOpen} onToggle={() => setMoreOpen(value => !value)} section={section} onSelect={onSelect}/>{user.role === 'ADMIN' && <><div className="my-3 h-px bg-slate-200 dark:bg-slate-800"/><CollapsibleNav label="Administração" items={adminNav} open={adminOpen} onToggle={() => setAdminOpen(value => !value)} section={section} onSelect={onSelect}/></>}</div>
...
export default function ShellV2({ user, section, onSection, onLogout, onSearch, children }: Props) {
  ...
  useHotkeys('ctrl+k, meta+k', event => { event.preventDefault(); const input = document.getElementById('cv-workspace-search') as HTMLInputElement | null; input?.focus(); input?.select(); }, { enableOnFormTags:true });
