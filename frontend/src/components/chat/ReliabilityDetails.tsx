      ...
      {sources.length ? (
        <div className="mt-3">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {sources.map(source => <span key={source} className="rounded-full bg-white px-2.5 py-1 text-[10px] font-semibold text-[#1d4f91] ring-1 ring-blue-100">{retrievalLabels[source as RetrievalSource]}</span>)}
          </div>
        </div>
      ) : null}
      {evidence.length ? (
