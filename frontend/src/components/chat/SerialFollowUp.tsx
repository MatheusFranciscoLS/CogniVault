...
export default function SerialFollowUp({ disabled, onSubmit }: { disabled: boolean; onSubmit: (serial: string) => void }) {
  ...
  return (
    <form onSubmit={submitSerial} className="mt-3 rounded-xl border border-amber-200 dark:border-amber-800 bg-white dark:bg-ink-800 p-3">
      <label className="block text-xs font-semibold text-ink-700 dark:text-ink-300" htmlFor="guided-serial-number">Digite o número de série da etiqueta</label>
      <p className="mt-1 text-[11px] leading-4 text-ink-500 dark:text-ink-400">Não é preciso repetir peça, modelo ou PNC. O CogniVault continuará a consulta anterior com este S/N.</p>
