...
export default function ResultCard({
  ...
  return (
    <div className="mt-3 rounded-2xl border border-slate-200 bg-white p-4 text-slate-800 shadow-sm transition hover:shadow-md">
      ...
      {response.b2bPortal && (
        <div className={`mt-3 rounded-xl border p-3 text-xs ${response.b2bPortal.success ? 'border-emerald-200 bg-emerald-50 text-emerald-900' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
          <b className="block">Status no B2B</b>
          <span className="mt-1 block">{response.b2bPortal.stockStatus}</span>
          {response.b2bPortal.supersededBy && <span className="mt-1 block text-rose-700 font-semibold">Substituído por: {response.b2bPortal.supersededBy}</span>}
        </div>
      )}

      <div className="mt-4 flex flex-wrap gap-2">
        <button type="button" disabled={favoritePending} onClick={onToggleFavorite} className={`rounded-xl border px-3 py-2 text-xs font-semibold transition hover:opacity-80 disabled:cursor-not-allowed disabled:opacity-50 ${favorite?'border-amber-300 bg-amber-50 text-amber-800':'border-slate-300 text-slate-700'}`}>{favorite?'★ Favoritada':'☆ Favoritar peça'}</button>
        <button type="button" onClick={onCopyCode} className="rounded-xl bg-[#1d4f91] px-3 py-2 text-xs font-semibold text-white transition hover:opacity-90">Copiar código</button>
        <button type="button" onClick={onCopySummary} className="rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold transition hover:bg-slate-50">Copiar ficha</button>
        <button type="button" onClick={() => onAccess('view')} className="rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold transition hover:bg-slate-50">Abrir na página</button>
        <button type="button" onClick={() => onAccess('download')} className="rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold transition hover:bg-slate-50">Baixar PDF</button>
      </div>
    </div>
  );
}
