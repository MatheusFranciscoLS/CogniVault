...
function confidencePresentation(response: ChatResponse) {
  ...
  const presentations = {
    ...
    HIGH: { label: 'Confiança alta', style: 'bg-brand-50 dark:bg-ink-900 text-brand-700 dark:text-brand-300 ring-brand-200' },
...
export default function ResultCard({
  ...
  return (
    <div className="mt-3 rounded-2xl border border-ink-200 dark:border-ink-700 bg-white dark:bg-ink-800 p-4 text-ink-800 dark:text-ink-200 shadow-sm transition hover:shadow-md">
      ...
      {response.match ? <div className="mt-3 rounded-xl bg-ink-50 dark:bg-ink-800/50 p-3 text-xs leading-5 text-ink-500 dark:text-ink-400">{response.match.explanation}</div> : null}
        ...
        <div className="mt-3 rounded-2xl border-2 border-brand-200 dark:border-brand-800/80 bg-gradient-to-br from-brand-50/90 via-indigo-50/40 to-ink-50 dark:from-brand-600/80 dark:via-ink-800/80 dark:to-ink-900/90 p-3.5 shadow-2xs">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-brand-600 dark:text-brand-300">
            ...
            <div className="flex items-center gap-1.5">
              {part.position && (
                <span className="rounded-lg bg-white dark:bg-ink-800 px-2.5 py-1 text-xs font-black text-brand-600 dark:text-brand-300 shadow-2xs border border-brand-200 dark:border-brand-700">
                  Pos. Nº {part.position}
                </span>
              )}
              {part.page && (
                <span className="rounded-lg bg-brand-600 text-white px-2.5 py-1 text-xs font-black shadow-2xs">
                  Pág. {part.page}
                </span>
              )}
            </div>
          ...
          <div className="mt-3 flex items-center justify-between gap-3 border-t border-brand-100 dark:border-brand-900/50 pt-2.5">
            <span className="text-[11px] font-medium text-ink-500 dark:text-ink-400 truncate max-w-[240px]">
              📄 {part.filename}
            </span>
            <button
              type="button"
              onClick={() => onAccess('view')}
              className="rounded-xl bg-brand-600 hover:bg-ink-900 text-white px-3 py-1.5 text-xs font-bold transition flex items-center gap-1.5 shadow-2xs active:scale-95 shrink-0"
            >
              <span>👁️</span>
      ...
      {response.technicalReasoningSteps && response.technicalReasoningSteps.length > 0 && (
        <details className="group mt-3 rounded-2xl border border-ink-200 dark:border-ink-700/80 bg-ink-50/60 dark:bg-ink-800/60 transition overflow-hidden">
          <summary className="flex cursor-pointer list-none items-center justify-between p-3 text-xs font-bold text-ink-700 dark:text-ink-300 hover:text-brand-600 dark:hover:text-brand-300 transition">
            <div className="flex items-center gap-2">
              <span className="text-sm">🧠</span>
              <span>Raciocínio Técnico Passo a Passo da IA</span>
              <span className="rounded-full bg-brand-100 dark:bg-brand-900/40 text-brand-600 dark:text-brand-300 px-2 py-0.2 text-[10px] font-black">
            ...
            {response.technicalReasoningSteps.map(step => (
              <div key={step.step} className="flex items-start gap-2.5 text-xs">
                <span className={`grid h-5 w-5 shrink-0 place-items-center rounded-full text-[10px] font-black ${
                  step.status === 'NOTICE'
                    ? 'bg-amber-400 text-ink-950 shadow-2xs'
                    : 'bg-emerald-500 text-white shadow-2xs'
                }`}>
                  {step.status === 'NOTICE' ? '★' : '✓'}
                </span>
                <div className="min-w-0">
                  <div className="font-bold text-ink-800 dark:text-ink-200">{step.title}</div>
                  <div className="mt-0.5 text-ink-600 dark:text-ink-400 leading-relaxed font-medium">{step.detail}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="mt-4 grid gap-2 text-xs sm:grid-cols-2">
        <div className="rounded-xl bg-ink-50 dark:bg-ink-800/50 p-3">Modelo<b className="mt-1 block">{part.model}</b></div>
        <div className="rounded-xl bg-ink-50 dark:bg-ink-800/50 p-3">PNC<b className="mt-1 block">{part.pnc || 'Não informado'}</b></div>
      ...
      {response.b2bPortal && (
        <div className={`mt-3 rounded-xl border p-3 text-xs ${response.b2bPortal.success ? 'border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-900 dark:text-emerald-300' : 'border-ink-200 dark:border-ink-700 bg-ink-50 dark:bg-ink-800/50 text-ink-600 dark:text-ink-400'}`}>
        ...
        <div className="mt-3 rounded-2xl border-2 border-amber-300 dark:border-amber-700 bg-gradient-to-br from-amber-50/90 via-orange-50/40 to-yellow-50/30 dark:from-amber-950/40 dark:via-ink-800 dark:to-ink-900 p-3.5 shadow-2xs">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <button
              ...
              className="rounded-xl bg-amber-500 hover:bg-amber-400 text-ink-950 px-3 py-1.5 text-xs font-bold transition shadow-2xs active:scale-95 shrink-0"
            >
                  ...
                  <button
                    ...
                    className={`rounded-lg px-2 py-1 text-[10px] font-bold transition shrink-0 ${
                      addonInCart
                        ...
                        : 'bg-amber-400 hover:bg-amber-300 text-ink-950'
                    }`}
                  >
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          className="rounded-xl border border-ink-300 dark:border-ink-600 bg-white dark:bg-ink-800 hover:bg-ink-50 dark:hover:bg-ink-700 px-3 py-2 text-xs font-semibold text-ink-700 dark:text-ink-300 transition flex items-center gap-1.5 active:scale-95"
        >
          <span>📋</span>
        ...
        <button type="button" onClick={() => onAccess('download')} className="rounded-xl border border-ink-300 dark:border-ink-600 px-3 py-2 text-xs font-semibold transition hover:bg-ink-50 dark:bg-ink-800/50">Baixar PDF</button>
      </div>
