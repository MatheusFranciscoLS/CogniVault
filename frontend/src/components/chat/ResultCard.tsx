...
function confidencePresentation(response: ChatResponse) {
  ...
  const presentations = {
    ...
    HIGH: { label: 'Confiança alta', style: 'bg-blue-50 dark:bg-[#123867] text-blue-700 ring-blue-200' },
...
export default function ResultCard({
  ...
  return (
    <div className="mt-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 text-slate-800 dark:text-slate-200 shadow-sm transition hover:shadow-md">
      ...
      {(part.applications?.length || 0) > 1 ? (
        <div className="mt-3 rounded-xl border border-blue-100 dark:border-blue-700 bg-blue-50 dark:bg-[#123867]/60 p-3">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {part.applications?.map(application => <span key={`${application.model}-${application.pnc}`} className="rounded-full bg-white dark:bg-slate-800 px-2.5 py-1 text-[10px] font-medium text-blue-800 ring-1 ring-blue-100">{application.model} · PNC {application.pnc}</span>)}
          </div>
        </div>
      ) : null}
      ...
      {response.b2bPortal && (
        <div className={`mt-3 rounded-xl border p-3 text-xs ${response.b2bPortal.success ? 'border-emerald-200 bg-emerald-50 text-emerald-900' : 'border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400'}`}>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button type="button" disabled={favoritePending} onClick={onToggleFavorite} className={`rounded-xl border px-3 py-2 text-xs font-semibold transition hover:opacity-80 disabled:cursor-not-allowed disabled:opacity-50 ${favorite?'border-amber-300 bg-amber-50 text-amber-800':'border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300'}`}>{favorite?'★ Favoritada':'☆ Favoritar peça'}</button>
