...
type Message = { role:'user'|'ai'; text:string; response?:ChatResponse; query?:string; pnc?:string; feedback?:'correct'|'wrong'|'corrected'; showReasons?:boolean; showCorrections?:boolean; reason?:FeedbackReason };
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_330px]">
      <div className="cv-surface overflow-hidden rounded-[24px]">
        ...
        <div className="cv-scrollbar min-h-[480px] max-h-[62vh] space-y-4 overflow-auto p-5">
          ...
          {messages.map((message,index)=><div key={index} className={message.role==='user'?'flex justify-end':'flex justify-start'}><div className={`max-w-[92%] whitespace-pre-line rounded-2xl px-4 py-3 text-sm ${message.role==='user'?'bg-[#1d4f91] text-white':'bg-slate-100 text-slate-800'}`}>
      ...
      <aside className="space-y-4">
        <div className="cv-surface rounded-[22px] p-5"><div className="text-sm font-semibold">Equipamentos salvos</div><p className="mt-1 text-xs text-slate-400">Reaplique modelo e PNC usados com frequência.</p><div className="mt-4 grid gap-2">{!equipment.length&&<div className="rounded-xl bg-slate-50 p-3 text-xs text-slate-400">Nenhum equipamento salvo.</div>}{equipment.map(item=><button key={item.id} onClick={()=>{setManufacturer(item.manufacturer);setModel(item.model);setPnc(item.pnc);notify('Equipamento aplicado.')}} className="rounded-xl border border-slate-200 p-3 text-left text-xs hover:bg-slate-50"><b className="block text-slate-700">{item.label}</b><span className="mt-1 block text-slate-400">Usar nesta busca</span></button>)}</div></div>
