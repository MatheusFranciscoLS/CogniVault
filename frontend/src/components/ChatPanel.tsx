        ...
        <div className="cv-surface overflow-hidden rounded-[24px]">
          <div className="grid gap-3 border-b border-slate-200 dark:border-slate-700 bg-slate-50/70 p-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:col-span-2 lg:col-span-4">
              <button type="button" onClick={saveEquipment} className="rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-xs font-semibold">☆ Salvar equipamento</button>
              {(manufacturer || model || pnc || serial) ? <button type="button" onClick={() => { setManufacturer(''); setModel(''); setPnc(''); setSerial(''); }} className="rounded-xl px-3 py-2 text-xs font-semibold text-slate-400 hover:bg-white dark:bg-slate-800 hover:text-slate-700 dark:text-slate-300">Limpar equipamento</button> : null}
            </div>
          </div>
          ...
          <div className="cv-scrollbar min-h-[480px] max-h-[62vh] space-y-4 overflow-auto p-5" aria-busy={loading}>
            {!messages.length ? (
              <div className="grid h-[420px] place-items-center text-center">
                <div className="max-w-lg">
                  <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-blue-50 dark:bg-[#123867] text-2xl text-[#1d4f91]">✦</div>
                  ...
                  <div className="mt-4 flex flex-wrap justify-center gap-2">
                    {quickPrompts.map(prompt => <button type="button" key={prompt} onClick={() => { setQuestion(prompt); questionRef.current?.focus(); }} className="rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-1.5 text-xs font-medium text-slate-600 dark:text-slate-400 transition hover:border-blue-200 dark:border-blue-600 hover:bg-blue-50 dark:bg-[#123867] hover:text-[#1d4f91]">{prompt}</button>)}
                  </div>
                </div>
              </div>
            ) : null}
            ...
            {messages.map((message, index) => (
              ...
              >
                <div className={`max-w-[94%] rounded-2xl px-4 py-3 text-sm shadow-sm ${message.role === 'user' ? 'bg-[#1d4f91] text-white rounded-br-sm' : 'bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-bl-sm border border-slate-200 dark:border-slate-700'}`}>
          ...
          <form onSubmit={submit} className="flex gap-3 border-t border-slate-200 dark:border-slate-700 p-4">
