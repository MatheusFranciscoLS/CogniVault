...
export default function Shell({ user, section, onSection, onLogout, onSearch, children }: Props) {
  ...
  const refreshNotifications = () => {
    void fetchNotifications().then(data => setNotifications(data.notifications)).catch(() => setNotifications([]));
  };

  useEffect(() => {
    let active = true;
    const refresh = () => {
      void fetchNotifications().then(data => { if (active) setNotifications(data.notifications); }).catch(() => { if (active) setNotifications([]); });
    };
    refresh();
  ...
  const renderNav = (items: NavItem[]) => items.map(([id, label]) => {
    ...
    return (
      <button type="button" key={id} onClick={() => select(id)} aria-current={active ? 'page' : undefined} className={`group flex w-full items-center gap-3 rounded-[13px] px-3 py-2.5 text-left text-sm transition ${active ? 'bg-[#edf3fb] font-semibold text-[#173f76] shadow-[inset_3px_0_0_#1d4f91]' : 'font-medium text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}>
        <span className={`grid h-7 w-7 place-items-center rounded-lg transition ${active ? 'bg-white text-[#1d4f91] shadow-sm' : 'text-slate-400 group-hover:text-slate-700'}`}><NavIcon section={id}/></span>
        <span>{label}</span>
      </button>
    );
  });
  ...
  const sidebar = (
    <aside className="flex h-full flex-col overflow-hidden border-r border-slate-200/80 bg-white text-slate-900">
      <div className="px-5 pb-4 pt-6">
        <img src="/vardao-logo-transparent.png" alt="Vardão Máquinas" className="h-auto w-[154px] object-contain" />
        <div className="mt-5 flex items-center gap-3 rounded-2xl border border-slate-200/80 bg-[#f8fafc] p-3">
          <img src="/favicon.png" alt="" className="h-9 w-9 rounded-xl object-cover shadow-sm" />
          <div className="min-w-0"><div className="text-sm font-semibold tracking-tight text-slate-900">CogniVault</div><div className="mt-0.5 text-[9px] font-bold uppercase tracking-[.16em] text-slate-400">Inteligência de peças</div></div>
        </div>
      </div>

      <nav aria-label="Navegação principal" className="cv-scrollbar min-h-0 flex-1 overflow-y-auto px-3 pb-4">
        <div className="px-3 pb-2 pt-2 text-[10px] font-bold uppercase tracking-[.14em] text-slate-400">Operação</div>
        <div className="grid gap-1">{renderNav(operationNav)}</div>
        {user.role === 'ADMIN' && <><div className="mx-3 my-4 h-px bg-slate-200/80" /><div className="px-3 pb-2 text-[10px] font-bold uppercase tracking-[.14em] text-slate-400">Administração</div><div className="grid gap-1">{renderNav(adminNav)}</div></>}
      </nav>

      <div className="border-t border-slate-200/80 p-4">
        <div className="mb-3 flex items-center gap-3 rounded-xl bg-[#f7f9fc] p-2.5">
          <img src="/husqvarna-logo.webp" alt="Husqvarna" className="h-8 w-8 rounded-lg object-cover shadow-sm" />
          <div><div className="text-[11px] font-semibold text-slate-700">Representante Husqvarna</div><div className="mt-0.5 text-[9px] text-slate-400">Assistência e peças</div></div>
        </div>
        <div className="flex items-center gap-3">
          <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#0d2348] text-[10px] font-bold text-white">{initials}</div>
          <div className="min-w-0 flex-1"><div className="truncate text-xs font-semibold text-slate-700">{user.email}</div><div className="mt-0.5 text-[9px] uppercase tracking-[.1em] text-slate-400">{user.role === 'ADMIN' ? 'Administrador' : 'Balcão'}</div></div>
          <button type="button" onClick={onLogout} className="rounded-lg px-2 py-1.5 text-[11px] font-semibold text-slate-400 transition hover:bg-rose-50 hover:text-rose-600">Sair</button>
        </div>
      </div>
    </aside>
  );
  ...
  return (
    <div className="min-h-screen bg-[#f4f7fb] lg:grid lg:grid-cols-[268px_1fr]">
      ...
      <main className="min-w-0">
        <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
          <div className="flex min-h-[68px] items-center gap-3 px-4 sm:px-6 md:px-8">
            <button type="button" aria-label="Abrir menu" aria-expanded={menuOpen} onClick={() => setMenuOpen(true)} className="cv-icon-button lg:hidden"><span aria-hidden="true" className="text-lg leading-none">☰</span></button>
            <div className="hidden min-w-[112px] xl:block"><div className="text-[10px] font-bold uppercase tracking-[.13em] text-slate-400">Área atual</div><div className="mt-0.5 text-sm font-semibold text-slate-800">{currentLabel}</div></div>
