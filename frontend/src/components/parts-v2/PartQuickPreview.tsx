...
export default function PartQuickPreview({ technical, commercial, verification, onCopy, onOpenTechnical, onCrossReference, onOfficial }: Props) {
  ...
  return <aside className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
    ...
    {technical ? <><div className="grid grid-cols-3 gap-2 text-[10px] text-slate-400"><span>PNC <strong className="block truncate text-slate-700 dark:text-slate-200">{technical.pnc || '—'}</strong></span><span>Pos. <strong className="block text-slate-700 dark:text-slate-200">{technical.position || '—'}</strong></span><span>Pág. <strong className="block text-slate-700 dark:text-slate-200">{technical.page || '—'}</strong></span></div><div className="rounded-lg border border-slate-100 bg-slate-50/70 p-3 dark:border-slate-800 dark:bg-slate-950/40"><div className="text-[8px] font-black uppercase tracking-[.12em] text-slate-400">Cadeia de evidência</div><div className="mt-2 flex flex-wrap items-center gap-1 text-[9px] font-bold text-slate-600 dark:text-slate-300">{evidenceSteps.map((step, index) => <span key={`${step}-${index}`} className="flex items-center gap-1"><span className="rounded bg-white px-1.5 py-1 shadow-sm dark:bg-slate-800">{step}</span>{index < evidenceSteps.length - 1 && <span className="text-slate-300">›</span>}</span>)}</div><div className={`mt-2 text-[9px] font-bold ${verification?.state === 'VERIFIED' || verification?.state === 'SUPERSEDED' ? 'text-emerald-600 dark:text-emerald-400' : verification?.state === 'REVIEW' ? 'text-amber-600 dark:text-amber-400' : 'text-slate-400'}`}>{officialEvidence(verification)}</div></div></> : null}
