...
export default function PartDetailDrawer({ detail, verification, verificationLoading = false, liveData, onClose, onCopy, onOpenPdf, onOpenRelated, onToggleFavorite, onVerify, onCrossReference, onAskAi }: Props) {
  ...
  const addToQuote = () => { recordQuoteUsage([...quoteCart.items, { partNumber: effectiveCode, model: detail.model }], quoteCart.items.length === 0); quoteCart.addItem({ partNumber: effectiveCode, effectiveCode, manufacturer: detail.manufacturer || 'Husqvarna', name: detail.name, model: detail.model, pnc: detail.pnc, section: detail.section, position: detail.position, filename: detail.filename, page: detail.page, isSuperseded: superseded, originalCode: superseded ? originalCode : undefined, notes: detail.notes, unitPrice: detail.price ?? undefined }); };
